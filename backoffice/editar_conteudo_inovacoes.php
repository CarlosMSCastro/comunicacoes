...
if (!$id) {
    ...
    $item = select_sql("SELECT * FROM paginas_inovacoes WHERE id = ?", [$id])[0] ?? null;
    if (!$item) {
...
if (isset($_POST['titulo'], $_POST['texto'], $_POST['texto_2'], $_POST['imagem'])) {
    ...
    if ($id) {
        ...
        $id_navbar = select_sql("SELECT id_navbar FROM paginas_inovacoes WHERE id = ?", [$id])[0]['id_navbar'];
        ...
        $paiSolucoes = select_sql("SELECT id FROM navbar WHERE titulo = 'Inovação e Tecnologia' LIMIT 1")[0]['id'] ?? null;
        if (!$paiSolucoes) die("Erro: submenu 'Inovação e Tecnologia' não encontrado!");
        ...
        // Inserir página
        $consulta2 = $pdo->prepare("INSERT INTO paginas_inovacoes (titulo_h1, texto, texto_2, imagem, id_navbar) VALUES (?, ?, ?, ?, ?)");
...
<div class="caixa">
    ...
    <form method="post">
        ...
        <a href="editar_inovacoes.php" class="btn btn-secondary">Cancelar</a>
    </form>
</div>
